...
with open('data/Revised_Trajectory_Label_Array.pickle', 'rb') as f:
...
for z in range(len(Trajectory_Label_Array)):
    ...
    Label = []
    min_trip_time = 30 * 60  # 20 minutes equal to 1200 seconds
    threshold = 300  # fixed of number of GPS points for each instance
    i = 0
    while i <= (len(Data) - 1):
        No = 0
        ModeType = Data[i, 3]
    ...
    for k in tqdm(range(len(InstanceNumber)), desc="%s" % (z + 1)):
        ...
        for i in range(len(Data) - 1):
            ...
            y = math.sin(math.radians(Data[i + 1, 1]) - math.radians(Data[i, 1])) * math.radians(
                math.cos(Data[i + 1, 0]))
...
with open('data/Revised_InstanceCreation+NoJerkOutlier+Smoothing.pickle', 'wb') as f:
