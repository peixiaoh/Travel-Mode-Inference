...
for k in tqdm(range(len(UserNon))):
    InputFile = "data/Combined Trajectory_Label_Geolife/combined" + str(UserNon[k]) + ".plt"
    table = []
    try:
    ...
    InputFile = "data/Combined Trajectory_Label_Geolife/labels" + str(UserNon[k]) + ".txt"
    table = []
    with open(InputFile, 'rb') as inp:
        for row in inp:
            row = row.rstrip()
...
with open("data/Revised_Trajectory_Label_Array.pickle", 'wb') as f:  # Python 3: open(..., 'wb')
